...
function show(data) {
  ...
  for (let i = 0; i <= 99; i++) {
    homeData.push({
      title: '这是标题' + i,
      content: '背景 接着前面的的表单元素继续说，用户提交一个表单或者一段数据的时候，' +
          '通常我们都会有表单验证，验证内容是否合法，验证内容的有效性，验证内容是否填写了等等，' +
          '这样的需求在平常的项目里面应该是一个很常见的操作。如果你对form表单有了解，我相信你很多事原生标签都帮你做了' + i,
      time: '2020-01-09' + i,
      classify: '分类' + i,
      id: ((Math.random() * 10) * (Math.random() * 10)) + i
    })
  }
