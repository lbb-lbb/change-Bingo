<template>
  <div class="card">
    <div class="list animate__animated animate__bounceInUp">
    ...
    <RightMenuLayout class="right-menu-layout animate__animated animate__bounceInDown">
...
export default {
  ...
  computed: {
    filterClassify: function() {
      if (this.data) {
        let filter = []
        filter = [...new Set(this.data.map(v => v.category))]
        return filter
      } else {
        return []
      }
    ...
    filterData: function() {
      ...
      this.filterClassify.forEach(val => {
        data[val] = this.data.filter(v => v.category === val)
      })
      return data
    }
  },
